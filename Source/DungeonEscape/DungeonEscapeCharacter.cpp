...
ADungeonEscapeCharacter::ADungeonEscapeCharacter()
{
	...
	FirstPersonCameraComponent->SetRelativeLocationAndRotation(FVector(0.f, 0.f, 70.f), FRotator::ZeroRotator);
...
{	
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		// Interact
		EnhancedInputComponent->BindAction(InteractAction, ETriggerEvent::Started, this, &ADungeonEscapeCharacter::InteractStart);
		EnhancedInputComponent->BindAction(InteractAction, ETriggerEvent::Completed, this, &ADungeonEscapeCharacter::InteractEnd);
	}
	else
	{
...
void ADungeonEscapeCharacter::InteractStart()
{
	// UE_LOG(LogTemp, Warning, TEXT("INTERACT START"));

	FVector LineStart = FirstPersonCameraComponent->GetComponentLocation();
	FVector LineEnd = LineStart + (FirstPersonCameraComponent->GetForwardVector() * MaxInteractionDistance);

	DrawDebugLine(GetWorld(), LineStart, LineEnd, FColor::Green, false, DebugDrawDuration);

	FCollisionShape SphereShape = FCollisionShape::MakeSphere(InteractionSphereRadius);
	DrawDebugSphere(GetWorld(), LineStart, InteractionSphereRadius, 20, FColor::Red, false, DebugDrawDuration);
	DrawDebugSphere(GetWorld(), LineEnd, InteractionSphereRadius, 20, FColor::Blue, false, DebugDrawDuration);
...
void ADungeonEscapeCharacter::InteractEnd()
{
